...
public:
    int longestIncreasingPath(const vector<vector<int>>& matrix) {
        ...
        for (int i = 0; i < m; i++) {
            for (int j = 0; j < n; j++) {
                max_len = max(max_len, depthFirstSearch(i, j, matrix));
            }
        }
        return max_len;
    }
...
private:
    ...
    int depthFirstSearch(int i, int j, const vector<vector<int>>& mat) {
        ...
        for (const auto& step : steps) {
            ...
            if ((x >= 0 && x < m) && (y >= 0 && y < n) && (mat[x][y] > mat[i][j])) {
                path[i][j] = max(path[i][j], depthFirstSearch(x, y, mat));
            }
        }
        return ++path[i][j];
    }
