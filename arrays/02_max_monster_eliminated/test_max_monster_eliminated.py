...
from max_monster_eliminated import Solution
...
@time_tests
class TestMaxMonsterEliminated(TestCase):
    def setUp(self):
        # This method is called before each test
        self.solution = Solution()

    def test1_LowSpeed_LongDistance(self):
        dist = [1, 3, 4]
        speed = [1, 1, 1]
        expected = 3
        result = self.solution.max_monster_eliminated(dist, speed)

        self.assertEqual(result, expected)

    def test2_LowSpeed_ShortDistance(self):
        dist = [1, 1, 2, 3]
        speed = [1, 1, 1, 1]
        expected = 1
        result = self.solution.max_monster_eliminated(dist, speed)
    ...
    def test3_HighSpeed_ShortDistance(self):
        dist = [3, 2, 4]
        speed = [5, 3, 2]
        expected = 1
        result = self.solution.max_monster_eliminated(dist, speed)
...
if __name__ == '__main__':
    suite = TestLoader().loadTestsFromTestCase(TestMaxMonsterEliminated)
